...
private_bucket = Notification(bucket="private-bucket", findings={"finding 1"})
public_bucket = Notification(bucket="public-bucket", findings={"finding 2"})
another_bucket = Notification(bucket="another-bucket")
unencrypted_bucket = Notification(bucket="unencrypted-bucket", findings={"finding 3"})
...
class TestNotificationFilter(TestCase):
    def test_filter(self) -> None:
        ...
        self.assertEqual({private_bucket}, NotificationFilter().do_filter(notifications, filters))

    def test_empty_filters(self) -> None:
        notifications = {private_bucket, public_bucket, unencrypted_bucket}
