...
from tests.fixtures.github_compliance_alerter import github_report

channel = "the-alerting-channel"
config = "the_config_bucket"
report = "the_report_bucket"
s3_report_key = "s3_audit.json"
report_key = "github_admin_report.json"
slack_api_url = "https://the-slack-api-url.com"
slack_username_key = "the-slack-username-key2"
slack_token_key = "the-slack-token-key2"
...
@mock_sts
@httpretty.activate
class TestGithubComplianceAlerter(TestCase):
    def setUp(self) -> None:
    ...
    def _setup_environment() -> None:
        patch.dict(
            environ,
            {
                ...
                "S3_AUDIT_REPORT_KEY": s3_report_key,
                "GITHUB_AUDIT_REPORT_KEY": report_key,
    ...
    def _setup_report_bucket() -> None:
        ...
        s3.put_object(Bucket=report, Key=report_key, Body=dumps(github_report))

    @staticmethod
    def _setup_config_bucket() -> None:
        s3 = boto3.client("s3")
        s3.create_bucket(Bucket=config)
        s3.put_object(Bucket=config, Key="filters/a", Body=dumps([{"item": "bad-repo-no-admin", "reason": "because"}]))
        s3.put_object(
